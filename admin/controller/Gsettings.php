...
elseif (route(2) == "general"):
    ...
    if ($access):
        if ($_POST):
            ...
            if (empty($name)):
                $errorText = "O nome do site não pode estar vazio.";
                $error = 1;
            elseif (empty($currency) or empty($csymbol)):
                $errorText = "A moeda e o símbolo não podem estar vazios.";
                $error = 1;
            else:
